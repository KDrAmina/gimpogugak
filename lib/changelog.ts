...
export const CHANGELOG: ChangelogEntry[] = [
  ...
  // ────────────────────────────────────────────────────────────────────────
  {
    ...
    date: "2026-04-15",
    changes: [
      "[통계] 전체 차트 툴팁 전역 동기화: 연도별/월별/YoY 모드별 syncId 분리(sync-year·sync-month·sync-yoy) — 동일 X축 모든 차트에서 hover 시 툴팁·가이드라인 일괄 표시",
      "[통계] 유입경로 트렌드 범례 통합: 중복 범례 제거 후 하단 요약 뱃지를 토글 버튼으로 통합 — 클릭 시 라인 On/Off, 마지막 1개 라인 보호, 기간 변경 시 자동 초기화",
      "[통계] 카테고리별 수강료 트렌드 토글: 우측 상단 카테고리 뱃지를 토글 버튼으로 전환 — 클릭 시 해당 라인 숨김/표시, 숨김 항목 취소선·반투명 표시",
    ],
  },
  {
    version: "5.5.8",
    date: "2026-04-15",
    changes: [
      "[통계] 매출 추이 ↔ 월별 외부수입 트렌드 차트 툴팁 동기화: 한 차트에 마우스를 올리면 같은 X축(월/연도)의 다른 차트에서도 툴팁·가이드라인 함께 표시",
      "[통계] 유입경로 트렌드 범례 클릭 토글: 범례 항목 클릭 시 해당 라인 숨김/표시 — 흐리기+취소선으로 상태 표시, 마지막 1개 라인은 보호(전체 숨김 방지)",
    ],
  },
  {
    version: "5.5.7",
    date: "2026-04-14",
    changes: [
      "[블로그] 무한 스크롤 → 숫자 페이지네이션으로 전환: 한 페이지 10개 표시, 하단 페이지 번호 네비게이션 추가",
      "[블로그] URL ?page=N 쿼리 파라미터로 현재 페이지 상태 유지 — 글 읽고 뒤로가기 시 이전 페이지 번호 복원",
      "[블로그] 탭·검색어 변경 시 페이지 1로 초기화, 페이지 이동 시 게시물 번호 전체 기준 연속 표시 유지",
    ],
  },
  {
    version: "5.5.6",
    date: "2026-04-10",
    changes: [
      "[UI] 수동 발송 아코디언 위치 이동: 페이지 최하단 → 안내 배너 아래, 수강생 테이블 바로 위 — 체크박스 선택 후 즉시 발송 가능하도록 동선 개선",
      "[알림톡] 토글 OFF 수강생 납부 완료 표시 추가: alimtalkEnabled=false라도 hasPaidThisMonth=true이면 '🔵 이번달 납부완료 (수동제외)' 파란 배지 표시 — 원장님이 장부상 납부 여부를 상세 페이지 진입 없이 목록에서 즉시 파악 가능",
      "[알림톡] 상태 배지 우선순위 최종 재정립: hasPaidThisMonth를 모든 조건보다 최우선으로 끌어올림 — 조건 A-1(납부O+토글OFF), A-2(납부O+발송O), A-3(납부O+발송X), B-1(납부X+0원), B-2(납부X+토글OFF), B-3(납부X+발송O) 순서로 명확히 분기",
    ],
  },
  {
    version: "5.5.5",
    date: "2026-04-10",
    changes: [
      "[버그수정] 알림톡 상태 배지 우선순위 전면 개편: hasPaidThisMonth + sentToday 동시 체크로 분기 — 문자 수신 후 납부(조건 A)·문자 수신 전 선결제(조건 B) 케이스를 각각 정확히 구분",
      "[알림톡] 조건 A (납부 O + 발송 이력 O): '🔵 이번달 납부완료 (발송 MM/DD)' 배지 — 문자를 받은 뒤 결제한 수강생은 발송일이 함께 표시됨",
      "[알림톡] 조건 B (납부 O + 발송 이력 X): '🔵 이번달 납부완료 (발송제외)' 배지 — 문자 발송 전 선결제한 수강생은 기존과 동일하게 표시됨",
      "[버그수정] 기존 버그 제거: hasPaidThisMonth만 체크하여 sentToday를 무시하던 로직 삭제 — '문자 수신 후 납부' 케이스가 '발송제외'로 잘못 덮어씌워지던 UI 렌더링 오류 해결",
    ],
  },
  {
    version: "5.5.4",
  ...
  },
  {
    ...
    date: "2026-04-08",
    changes: [
      "[알림톡] 이번 달 납부 완료 수강생 UI 표시: lesson_history 기반으로 hasPaidThisMonth 판별, '🔵 이번달 납부완료 (발송제외)' 배지 추가",
      "[알림톡] 0원 수강생 배지 개선: 결제일 여부와 관계없이 항상 '발송 제외(0원)' 배지 표시 (기존: isToday일 때만 표시)",
      "[알림톡] 납부 완료 수강생 토글 비활성화: hasPaidThisMonth=true이면 토글 흐림 처리 및 클릭 방지, 실수 수동 발송 방지",
      "[알림톡] 오늘 발송 예정 카운트(todayCount) 및 '전원 발송 완료' 판정에서 이번 달 납부 완료 수강생 제외",
      "[알림톡] 크론 테스트 결과에 납부완료 제외 건수(skippedAlreadyPaid) 표시 추가",
    ],
  },
  {
    version: "5.5.2",
  ...
  },
  {
    ...
    date: "2026-04-07",
    changes: [
      "[알림톡] 크론잡 0원 수강생 처리 개선: 스킵 시 notification_log에 status='skipped_zero_tuition'으로 기록, 크론 응답에 skippedZero 카운트 추가",
      "[알림톡] 테스트 발송 결과 배지 개선: DB 기록 기반으로 '오전 10시 자동 발송 완료 (성공: X건, 0원 제외: Y건)' 형식으로 팩트 표시",
      "[알림톡] 수동 발송 직후 목록 자동 새로고침 추가 — 발송완료 배지가 즉시 반영됨",
      "[알림톡] 발송완료 배지 형식 통일: 수동/자동 모두 '🟢 수동발송 (MM/DD)' / '🟢 발송완료 (MM/DD)' 형식으로 표시",
      "[알림톡] 발송 이력 로직 주석 상세화: 이번달 전체 조회 기반으로 과거 수동 발송도 배지 고정 표시되는 원리 설명",
    ],
  },
  {
    version: "5.5.0",
    date: "2026-04-06",
    changes: [
      "[통계] 대시보드 위젯 상/하단 가로 비율(Grid) 동기화: 매출 추이·외부수입 트렌드 col-span-2, 도넛 차트 col-span-1로 통일하여 여백 제거",
      "[통계] 차트 YoY(Year-over-Year) 연도별 겹쳐보기 기능 전면 적용: 매출 추이·외부수입 트렌드·신규 유입 추이 모두 연도별 겹쳐보기 지원, 카테고리별 수강료 트렌드에 카테고리 선택 탭(어린이 개인/어린이 단체/성인 개인/성인 단체) 추가",
      "[통계] 1억 원 이상 대단위 금액 'O.O억 원' 포맷팅 공통 유틸(fmtAmount) 적용: 수입 구성 분석 합계·외부수입 파이프라인 합계·KPI 카드 전체 적용",
      "[수강생] 상세페이지 이번 달 납부 여부 버그 픽스: UTC toISOString 대신 로컬 KST 연월 기준으로 판별, lesson_history 결제 완료 기록 기준으로 비교하도록 수정",
      "[통계] VIP 위젯 2단 분할 개편: 좌측 결제 총액 순위 TOP 10 유지, 우측 장기 수강생(총 납부 횟수) TOP 10 신설 — 이름·납부 횟수·최초 등록 연월 표시, 우리인력→민경임 normalizeName 적용",
    ],
  },
  {
    version: "5.4.0",
    date: "2026-04-05",
    changes: [
      "[통계] 수입 목표 달성률: 연필 아이콘 클릭으로 목표 금액 인라인 수정 가능 (localStorage 연도별 저장)",
      "[통계] 카테고리별 수강료 트렌드 LineChart 신규 추가 (어린이개인·어린이단체·성인개인·성인단체 4선)",
      "[통계] 카테고리 현황 위젯을 매출 추이 차트 바로 위로 이동 (기존 카드 → LineChart 전면 개편)",
      "[통계] 외부수입 전용 2단 분석 Row 추가: 월별 외부수입 트렌드 LineChart + 파이프라인 도넛",
      "[통계] 레이아웃 배치 순서 재편: ① 카테고리 트렌드 → ② 매출 추이 → ③ 외부수입 분석 → ④ 신규/이탈 → ⑤ VIP",
      "[통계] VIP TOP 10 전체 너비(full-width)로 단독 배치",
      "[통계] CategoryTrendChart·ExternalTrendChart 신규 컴포넌트 추가",
    ],
  },
  {
    version: "5.3.0",
    date: "2026-04-05",
    changes: [
      "[알림톡] 금요일 선발송 로직 추가: 금요일에 오늘(금)+토+일 결제 대상자 한 번에 선발송",
      "[알림톡] 토·일요일 크론 즉시 종료: 금요일 선발송 완료 후 주말에는 발송 없음",
      "[알림톡] CRON_SECRET 인증 분기: Vercel 자동 크론(Bearer 토큰 필수) vs 관리자 수동 발송(세션 인증)으로 분리",
      "[알림톡] test-alimtalk 라우트: CRON_SECRET 미설정 시에도 관리자 세션 유효하면 수동 발송 가능(개발 환경 지원)",
      "[알림톡] 크론 템플릿 #{결제일}: 금요일 선발송 시 실제 결제 예정일(토/일 날짜) 표시",
      "[알림톡] UI 오늘 자동 발송 예정 카운트: 요일별 동적 계산 (토·일=0, 금=금+토+일 합산, 월~목=오늘만)",
      "[알림톡] UI 말일 보정 로직: payDay가 해당 월 마지막 날 초과 시 말일로 처리 (matchesPayDay 함수)",
      "[알림톡] UI 토·일요일 로그 조회: 직전 금요일 sent_date로 조회하여 금요일 발송 결과 표시",
      "[알림톡] UI 배지 개선: 🟠 금요일 선발송 대상 / 🟢 금요일 발송완료 / ⚠️ 금요일 미발송 추가",
      "[알림톡] UI 주말 안내 배너 추가 및 isWeekendTarget 플래그로 보라색 행 하이라이트",
    ],
  },
  {
    version: "5.2.0",
  ...
  },
  {
    ...
    date: "2026-04-04",
    changes: [
      "통계 대시보드 기간 필터 추가: 전체 기간, 2023, 2024, 2025, 2026년 토글 버튼",
      "전체 기간 선택 시 연도별 4개 막대, 특정 연도 선택 시 월별 12개 막대로 차트 자동 전환",
      "데이터 페치 단순화: 6개 쿼리를 3개로 통합, 클라이언트 필터링으로 전환",
      "전년 대비 증감 카드: all이면 25년vs24년, 특정 연도면 해당연도 vs 전년도 비교",
      "VIP TOP 10, 외부수입 파이프라인, 신규이탈 차트 모두 선택 기간 기준으로 즉시 반영",
    ],
  },
  {
    version: "4.8.1",
    date: "2026-04-03",
    changes: [
      "관리자 홈 시스템 상태 위젯을 통계 대시보드 바로가기 버튼으로 교체",
      "통계 대시보드 요약 카드 6종으로 확장: 활성 수강생 수, 12개월 평균 월매출 추가",
      "수입 구성 분석 도넛 차트 신규 추가: 수강료/체험비/강사수수료/기타 비중 시각화",
      "수강생 카테고리 현황 섹션 추가: 어린이/성인 x 개인/단체 인원 및 월 수강료 표시",
      "하이라이트 배너 추가: 12개월 최고 매출월 및 최저 매출월 자동 표시",
    ],
  },
  {
    version: "4.8.0",
  ...
  },
  {
    ...
    date: "2026-04-03",
    changes: [
      "24년 3~6월, 25년 3월 정규수업 수입 누락 수동 보정 완료",
      "윤미호 성인개인(20만) 수업 신규 등록: 2024년 3/4/5/6월 각 20만원 결제이력 4건 삽입",
      "유지연 25년 3월(80만) 결제이력 누락 추가: 마이그레이션이 2월분만 처리하고 3월분 누락한 버그 수정",
      "고인경 프로필/수업/3월 이력(10만) 신규 생성: 마이그레이션에서 완전 누락된 수강생 수동 복구",
    ],
  },
  {
    version: "4.7.30",
    date: "2026-04-03",
    changes: [
      "고영숙 계정 단일화: 고영숙2 삭제, 1개 계정으로 통합",
      "성인단체 선납 정상화: 분기별 일시불 240,000원 -> 월 80,000원 x 3개월 선납(prepaid_month) 27건으로 재등록",
      "선납 납부월 기준 집계: 2024-12 납부분 2025년 1~3월로, 2025-03 납부분 4~6월로 매핑 완료",
    ],
  },

  {
    ...
    date: "2026-04-03",
    changes: [
      "마이그레이션 수입 누락 3종 완벽 복구: 활성 수강생 26년 1~2월 처리 재개(3~4월만 스킵), 외부24년 대금/해금 필터 폐기(25/26년과 동일 규칙 적용), 금액 셀 텍스트 혼합('20만원' 등) 정규식 파싱 강화",
      "수강생 관리 페이지 UI 필터링: 활성 수업이 있는 수강생만 표시 (과거 수강생 및 미등록 회원 자동 제외)",
    ],
  },
  {
    version: "4.7.26",
    date: "2026-04-03",
    changes: [
      "마이그레이션 롤백 로직 완전 재설계: '최근 N시간' 상대 기준 → '2026-04-02 00:00 KST' 절대 시간 기준으로 전환, 누적 쓰레기 데이터 전량 삭제",
      "외부수입 중복 제거 로직 추가: date+type+amount+description 동일 항목 1건만 INSERT (하루 2개 중복 방지)",
      "과거 수강생 일괄 승인 처리: 마이그레이션된 모든 프로필 status='active' 강제 업데이트 (관리자 승인 대기 목록 자동 정리)",
    ],
  },
  {
    version: "4.7.25",
    date: "2026-04-02",
    changes: [
      "마이그레이션 치명 버그 2종 픽스: lessons INSERT 시 존재하지 않는 status 컬럼 완전 제거 (is_active만 사용)",
      "외부24년 파싱 규칙 교체: 대금·해금 포함 행만 강사수수료로 처리 (나머지 전량 버림), 25/26년 분류 로직 유지",
      "롤백 로직 강화: external_income 삭제 창을 3시간으로 단축, 오늘 생성된 is_active=false lessons 안전장치 삭제 추가",
    ],
  },
  {
    version: "4.7.24",
    date: "2026-04-02",
    changes: [
      "마이그레이션 스크립트 완전 재작성: 핀셋 롤백 4종(26년 lesson_history/외부수입/이율이윤 오작동/유지연 is_active 버그) 정밀 삭제",
      "외부수입 파싱 전면 개편: 강사수수료만 잡던 로직을 체험비·기타·강사수수료 전 카테고리 파싱으로 확장 (키워드 기반 자동 분류)",
    ],
  },
  {
    version: "4.7.23",
    date: "2026-04-02",
    changes: [
      "마이그레이션 완전 재작성(rollback_and_migrate.ts): 자동 롤백(클렌징) + 5대 치명 버그 완벽 픽스",
      "이율/이윤 잘못된 프로필(010-2718-0838) 자동 삭제 및 기존 올바른 프로필(010-9968-3256)에 lesson_history만 삽입",
      "현재 활성 수강생 26년 엑셀 데이터 완전 스킵으로 900만원 수입 폭증 버그 차단",
      "신규 프로필 status=active 명시로 승인 대기 버그 수정, 유지연 25년 종료됨 강제, 외부수입 날짜 ISO 형식 보장",
    ],
  },
  {
    version: "4.7.22",
    date: "2026-04-02",
    changes: [
      "마이그레이션 스크립트 PK 중복 에러 픽스: 프로필 생성 시 고유 UUID 명시적 발급 및 upsert 처리",
      "마이그레이션 멱등성(중복 실행 안전) 보장: 수강 이력 INSERT 전 기존 종료된 수업 자동 삭제, 외부수입 INSERT 전 강사수수료 데이터 자동 초기화",
    ],
  },
  {
    version: "4.7.20",
  ...
  },
  {
    ...
    changes: [
      ...
      "[홈페이지] 신뢰 지표 바 추가: 원장/부원장 직강, 1:1 맞춤 커리큘럼,",
