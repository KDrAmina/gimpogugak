...
export const CHANGELOG: ChangelogEntry[] = [
  ...
  // ────────────────────────────────────────────────────────────────────────
  {
    ...
    changes: [
      ...
      "[통계] 차트 YoY(Year-over-Year) 연도별 겹쳐보기 토글 전체 차트 적용: 매출 추이·외부수입 트렌드·신규 유입 추이 모두 연도별 겹쳐보기 지원",
      "[통계] YoY 겹쳐보기 — 카테고리별 수강료 트렌드: 카테고리 선택 탭(어린이 개인/어린이 단체/성인 개인/성인 단체) 추가, 선택한 카테고리의 연도별(23~26년) 수강료 선 비교",
      "[통계] 대시보드 총액 1억 원 이상 시 'O.O억 원' 단위 포맷팅 적용: 수입 구성 분석 도넛 합계·외부수입 파이프라인 합계 및 전체 금액 표시 개선",
    ],
  },
  {
    version: "5.4.0",
