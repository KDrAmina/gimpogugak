...
export const CHANGELOG: ChangelogEntry[] = [
  ...
  // ────────────────────────────────────────────────────────────────────────
  {
    ...
    date: "2026-09-22",
    changes: [
      "[통계] 연도별 겹쳐보기 — 매출 추이를 수강료/외부수입으로 분리하고 지표 토글 추가. 기존 총매출의 두 구성요소(수강료·외부수입)를 합치지 않고 각각 표시하며, 툴팁에 연도별 수강료·외부수입·합계(=기존 총매출)를 함께 표시. 색상=연도, 실선=수강료 / 점선=외부수입",
      "[통계] 연도별 겹쳐보기 — 신규 유입 vs 이탈에 신규/이탈 지표 토글 추가. 연도 토글과 조합해 예) 2025·2026년 신규만 비교 가능. 지표도 마지막 1개는 끌 수 없음(연도 토글과 동일 정책)",
      "[통계] 연도별 겹쳐보기 — 월별 외부수입 트렌드·카테고리별 수강료 트렌드에도 동일한 연도 토글 칩 적용. 숨긴 항목이 남아 있던 Recharts 기본 범례는 토글 칩으로 대체",
      "[통계] 유입경로 트렌드 — 제목을 '유입경로 트렌드'로 통일하고, 연도별 겹쳐보기에서는 X축을 1~12월로 바꿔 연도별 월간 유입경로를 겹쳐 비교. 유입경로 색상은 그대로 두고 연도는 선 패턴(올해 실선)으로 구분하며 연도 토글 제공. 누적 보기(연도별 합산)는 변경 없음",
      "[개선] 겹쳐보기 차트 공용 컴포넌트(YoyMatrixChart) 도입 — 모든 겹쳐보기 차트가 같은 토글 칩·표 형식 툴팁을 사용하며, 툴팁에는 현재 화면에 보이는 연도·지표만 표시",
    ],
  },
  {
    version: "5.27.0",
    date: "2026-09-22",
    changes: [
      "[통계] 매출 추이(연도별 겹쳐보기)에 연도 토글 칩 추가 — 연도별 색상 범례를 겸하며 클릭으로 연도를 숨기거나 복원해 원하는 연도끼리(예: 2025·2026) 비교 가능. 기존 카테고리·유입경로 범례 칩과 동일한 스타일·동작(마지막 1개 보호), 칩 색상은 선 색상과 같은 YEAR_COLORS 사용",
      "[통계] 신규 유입 vs 이탈 추이(연도별 겹쳐보기)를 연도별 신규·이탈 동시 비교로 개선 — 기존에는 신규만 표시되고 연도 범례가 없었음. 색상=연도, 실선·채운 점=신규 / 점선·빈 점=이탈로 구분하고 연도 토글 칩 추가(연도 1개 클릭 시 그 해의 신규·이탈 선을 함께 숨김). 이탈은 누적 보기와 동일한 기준(종료월, 미설정 시 마지막 결제월)",
      "[수정] 누적 보기 매출 추이 Y축 금액 라벨 앞자리 잘림(`000만`) 수정 — 차트 left margin이 음수(-10)라 Y축이 SVG 밖으로 밀려나고 축 폭(44px)도 라벨보다 좁았음. margin 0·축 폭 56px로 조정하고 라벨을 `1,500만`·`1.2억` 형식으로 표시",
    ],
  },
  {
    version: "5.26.0",
  ...
  },
  {
    ...
    date: "2026-07-12",
    changes: [
      "[수정] 예약 발행 글이 발행 시각이 지나도 404로 남는 문제 해결 — 블로그 목록/상세 페이지를 ISR(60초 재검증)로 전환하여 예약 시각 도달 후 최대 1분 내 자동 노출",
      "[수정] blog-publish 크론 조회 범위를 11분 → 25시간으로 확대 — 크론이 하루 1회(Hobby 플랜)로 변경된 후에도 조회 범위가 10분 주기 기준으로 남아 있어 예약글 재검증이 누락되던 문제 해결",
      "[개선] blog-publish 크론에서 발행 완료된 예약글 IndexNow 색인 요청 자동 전송 (저장 시점에 생략된 색인 보완)",
    ],
  },
  {
    version: "5.22.0",
    date: "2026-05-12",
    changes: [
      "[개선] 통계 대시보드 — 월 평균 매출 분모 동적 계산 적용: 조회 연도가 현재 연도면 현재 달(getMonth()+1), 과거 연도면 12를 자동 적용하여 하드코딩 제거",
    ],
  },
  {
    version: "5.21.0",
    date: "2026-05-04",
    changes: [
      "[개선] 통계 — 이탈 수강생 누적 결제액 구간 차트 세분화: 기존 5구간(200만↑ 단일)을 13구간으로 확장 (50만 단위 4개 + 100만 단위 8개 + 1천만↑)",
      "[개선] 통계 — 누적 결제액 구간 바 차트 X축 레이블 회전(-40°) 및 바 폭 축소로 13개 구간이 겹치지 않게 표시",
    ],
  },
  {
    version: "5.20.0",
    date: "2026-05-04",
    changes: [
      "[기능] 수강생 상세 페이지 — 등록일 ✏️ 버튼 클릭 시 날짜 수정 모달 표시, DB(profiles.created_at) 즉시 업데이트",
      "[기능] 수강생 상세 페이지 — 결제 이력 각 항목에 ✏️ 버튼 추가, 클릭 시 결제일 수정 모달 표시, DB(lesson_history.completed_date) 즉시 업데이트 및 화면 즉시 반영",
      "[목적] 선결제로 인한 통계·미납 관리 오류 방지를 위해 관리자가 날짜를 직접 교정할 수 있도록 개선",
    ],
  },
  {
    version: "5.19.0",
    date: "2026-04-30",
    changes: [
      "[버그수정] 말일 미납자 브리핑 발송 시점 동적 최적화 — day === 31 고정 상수 제거, new Date(y, m+1, 0).getDate()로 실제 말일 동적 감지",
      "[버그수정] 말일이 주말인 경우 조기 반환으로 미납 브리핑이 누락되던 문제 수정 — 주말 early return 경로에 isLastDayOfMonth 가드 추가",
      "[버그수정] 오늘 결제 대상 없음/전원 납부 완료 early return 경로에도 isLastDayOfMonth 가드 추가 — 어떤 경로로 반환되더라도 말일 브리핑 누락 방지",
    ],
  },
  {
    version: "5.18.0",
    date: "2026-04-27",
    changes: [
      "[기능] 블로그 게시글 하단에 브랜딩 텍스트 섹션 고정 추가 — 학원명·지역·장르·무형문화재 문구 자동 표시",
      "[기능] 블로그 게시글 하단에 '1회 무료체험 신청하기!' 배너 버튼 고정 추가 — 클릭 시 네이버 톡톡 연결",
      "[UX] 무료체험 배너 클릭 GA 이벤트(click_free_trial) 추적 추가",
      "[디자인] 브랜딩 섹션 → 무료체험 배너 → 지도 순서로 배치, 섹션 간 일정 간격 유지",
    ],
  },
  {
    version: "5.17.0",
  ...
  },
  {
    ...
    date: "2026-04-24",
    changes: [
      "[버그수정] 모바일에서 /admin/login 진입 시 흰 화면 문제 수정 — AdminLayout의 !isAdmin 체크가 로그인 페이지보다 먼저 실행되어 null 반환되던 문제 해결",
      "[기능] /admin/login 페이지를 실제 관리자 로그인 폼으로 교체 (전화번호 + 비밀번호, 관리자 권한 검증)",
      "[UX] 로그인 페이지에 Suspense + 스피너 적용으로 초기 렌더링 즉시 표시",
      "[기능] 미들웨어에서 미인증 사용자 /admin/* 리다이렉트 시 ?next= 파라미터 전달 — 로그인 후 원래 페이지로 복귀",
    ],
  },
  {
    version: "5.16.0",
    date: "2026-04-24",
    changes: [
      "[UI] 관리자 상단 네비게이션 정리 — '회원관리', '미납관리', '설정' 메뉴 제거, 핵심 메뉴만 유지",
      "[UI] 홈 대시보드에 관리 바로가기 3열 카드 섹션 추가 (회원 관리 · 미납 관리 · 설정)",
      "[UX] 각 카드에 호버 효과 및 색상 강조 적용 — 기존 위젯과 디자인 톤 통일",
    ],
  },
  {
    version: "5.15.1",
    date: "2026-04-24",
    changes: [
      "[버그수정] 관리자 설정 페이지 계좌번호 저장 시 duplicate key 에러 수정 — upsert onConflict 옵션 명시",
      "[UX] 설정 저장 실패 시 원인별 친절한 한국어 에러 메시지 표시 (테이블 미존재·권한 없음 등)",
    ],
  },
  {
    version: "5.15.0",
    date: "2026-04-24",
    changes: [
      "[기능] 미납자 관리 페이지(/admin/billing/unpaid) 추가 — 당월 결제 완료 기록 없는 수강생 자동 추출",
      "[기능] 미납자 관리 페이지에서 전체/개별 체크박스로 선택 후 독촉 알림톡 일괄 발송",
      "[알림톡] 매월 말일 오전 10시 텔레그램으로 미납 인원 요약 브리핑 자동 발송",
      "[알림톡] 텔레그램 말일 브리핑 메시지 하단에 미납자 관리 페이지 인라인 버튼 포함",
      "[알림톡] Solapi PFID · 당월 결제 안내(일반) · 말일 리마인드(독촉) 템플릿 ID 분리 적용",
      "[알림톡] 모든 알림톡 발송 시 #{계좌번호} 변수 자동 매칭 — settings DB에서 실시간 조회",
      "[설정] 관리자 설정 페이지(/admin/settings) 추가 — 학원 계좌번호 DB 저장·수정",
      "[DB] settings 테이블 추가 — key-value 형태의 관리자 설정 저장소 (bank_account 등)",
      "[UI] 관리자 네비게이션에 '미납관리', '설정' 메뉴 추가",
    ],
  },
  {
    version: "5.14.0",
  ...
  },
  {
    ...
    date: "2026-04-23",
    changes: [
      "[UI] 소식 관리 수정 완료 후 사용자 블로그 페이지 대신 관리자 목록으로 자동 복귀",
      "[UI] 수정 버튼 클릭 시 현재 페이지 번호를 쿼리 파라미터로 전달 — 수정 완료 후 원래 페이지로 정확히 복귀 (PostEditor 내부에서 useSearchParams로 직접 읽어 안정성 확보)",
      "[UI] 수정 완료 시 '수정이 완료되었습니다. 목록으로 돌아갑니다.' 토스트 메시지 표시",
    ],
  },
  {
    version: "5.12.0",
    date: "2026-04-22",
    changes: [
      "[UI] 수강생 상세 페이지 '기본 정보' 섹션에 유입 경로(inflow_route) 항목 추가 — 미설정 시 '-' 표시",
      "[기능] 수강생 상세 페이지에서 유입 경로를 드롭다운으로 직접 수정·저장 가능 — DB 즉시 반영",
      "[통계] 상세 페이지에서 수정한 유입 경로가 대시보드 '유입 경로 트렌드' 차트에 실시간 연동",
    ],
  },
  {
    version: "5.11.0",
    date: "2026-04-20",
    changes: [
      "[알림톡] 텔레그램 일일 브리핑 포맷 개선 — 오늘 전체 결제 대상자 명단을 먼저 표시",
      "[알림톡] 발송 성공/제외(납부완료·0원)/실패를 각 항목별 인원수와 이름으로 구분 표시",
    ],
  },
  {
    version: "5.10.0",
  ...
  },
  {
    ...
    date: "2026-04-16",
    changes: [
      "[DB] lessons 테이블에 end_month(TEXT, YYYY-MM) 컬럼 추가 — 수업 종료(이탈) 연월 기록",
      "[DB] 마이그레이션: 기존 종료 수강생 end_month 일괄 자동 설정 — 마지막 결제월 + 1개월",
      "[통계] 신규 유입 vs 이탈 추이 차트 이탈 기준을 end_month(종료월)로 전면 변경 — 미설정 시 마지막 결제월 fallback",
      "[통계] 이탈 분석(churnStats) 필터·월별 이탈률 모두 end_month 기준으로 수정",
      "[UI] 수강생 상세 페이지 수업 종료 시 이탈월 입력 모달 추가 — 기본값: 마지막 결제월 + 1개월",
      "[UI] 이미 종료된 수업의 이탈월을 클릭 한 번으로 언제든 수정 가능",
      "[UI] 타임머신(과거 이력 일괄 등록) 생성 시 end_month 자동 설정",
    ],
  },
  {
    version: "5.8.0",
    date: "2026-04-16",
    changes: [
      "[UI] 수강생 관리 테이블에 No. 순번 컬럼 추가 — 정렬 순서 기반 절대 번호 표시",
      "[UI] 수업 관리 테이블(데스크탑·모바일)에 No. 순번 표시 — 현재 수업 중 인원 한눈에 파악",
      "[기능] is_test=true 계정 카테고리 0개 저장 허용: 수강생 관리(handleSaveStudentInfo) 및 수업 관리(handleSaveCategory) 유효성 검사 예외 처리",
      "[집계] 이름에 쉼표(,) 포함 시 개별 인원으로 카운트: 수강생 관리 상단 '전체 N명' 및 수업 관리 탭 집계 모두 적용",
    ],
  },
  {
    version: "5.7.0",
  ...
  },
  {
    ...
    date: "2026-04-15",
    changes: [
      "[버그수정] 이탈 분석 연도 필터 정상화: churnStats useMemo에 selectedYear 의존성 추가 — 이탈자 판별 기준을 'lastMonth가 선택 연도에 속하는지'로 변경, 바 차트도 연도 선택에 즉시 반응",
      "[통계] 이탈 KPI 신규 3종 추가(기존 '이탈 수강생 수' 카드 대체): ① 3개월 유지율 — 신규 등록자 중 3개월↑ 또는 50만원↑ 유지 비율, ② 월간 평균 이탈률 — 기간 내 매달 재적 대비 이탈자 비율 평균, ③ 이탈 전 평균 수강 개월 — 이탈자의 첫~마지막 결제 기간 평균",
      "[통계] KPI 카드 인라인 툴팁 추가: 각 지표 제목 옆 ⓘ 아이콘에 마우스를 올리면 지표 정의와 경영 해석 가이드 표시",
    ],
  },
  {
    version: "5.6.0",
    date: "2026-04-15",
    changes: [
      "[통계] 이탈 수강생 누적 결제액 분석 추가: is_active=false 수업 기준으로 수강생별 총 결제액을 집계, 이탈 수강생 수 및 평균 누적 결제액 KPI 카드 표시",
      "[통계] 이탈 분포 바 차트 추가: 50만 미만 / 50~100만 / 100~150만 / 150~200만 / 200만 이상 구간별 이탈자 수 시각화, 마우스 오버 시 정확한 이탈자 수 툴팁 표시",
    ],
  },
  {
    version: "5.5.9",
